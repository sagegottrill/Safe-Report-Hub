...
// Google Apps Script Webhook URL - Replace with your deployed webhook URL
const GOOGLE_APPS_SCRIPT_WEBHOOK_URL = 'YOUR_GOOGLE_APPS_SCRIPT_WEBHOOK_URL_HERE';
...
const ReportForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const webhookData = {
      ...
      location: `${formData.latitude},${formData.longitude}` || 'Unknown',
    ...
    if (navigator.onLine) {
      try {
        ...
        if (GOOGLE_APPS_SCRIPT_WEBHOOK_URL !== 'YOUR_GOOGLE_APPS_SCRIPT_WEBHOOK_URL_HERE') {
      ...
      if (navigator.onLine) {
        ...
        for (const key of offlineKeys) {
          ...
          if (report) {
            ...
            if (GOOGLE_APPS_SCRIPT_WEBHOOK_URL !== 'YOUR_GOOGLE_APPS_SCRIPT_WEBHOOK_URL_HERE' && report.webhookData) {
              try {
                const webhookResponse = await fetch(GOOGLE_APPS_SCRIPT_WEBHOOK_URL, {
                  method: 'POST',
                  headers: {
                ...
                console.error('Failed to sync offline report to webhook:', error);
              }
            }
