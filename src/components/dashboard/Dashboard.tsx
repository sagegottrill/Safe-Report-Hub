  ...
  return (
    <div className="p-3 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          ...
          <CardContent className="p-3 md:p-6 pt-0">
            ...
            <p className="text-xs text-blue-600">Your submitted reports</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
          ...
          <CardContent className="p-3 md:p-6 pt-0">
            ...
            <p className="text-xs text-green-600">Completed cases</p>
          </CardContent>
        </Card>
      </div>
                ...
                <div key={report.id} className="border rounded-lg p-2 md:p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex justify-between items-start mb-1 md:mb-2">
                    ...
                    <Badge className={`${getStatusColor(report.status)} text-xs`}>
                      {report.status ? report.status.replace('-', ' ') : t('Unknown')}
                    </Badge>
                  </div>
                  <p className="text-xs md:text-sm text-gray-600 mb-1 md:mb-2">
                    {report.description ? report.description.substring(0, 60) : t('No description')}...
                  </p>
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    <span className="truncate">{report.platform || t('Unknown platform')}</span>
                    <span className="text-xs">{report.id ? report.id.substring(0, 6) : t('N/A')}</span>
                    <span className="text-xs">{report.date ? new Date(report.date).toLocaleDateString() : t('N/A')}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card className="bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200">
        <CardHeader className="p-3 md:p-6">
          <CardTitle className="text-purple-800 text-sm md:text-base">Support Resources</CardTitle>
        ...
        <CardContent className="p-3 md:p-6 pt-0">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
            <div className="space-y-1 md:space-y-2">
              <h4 className="font-medium text-purple-800 text-xs md:text-sm">Crisis Support</h4>
              <p className="text-xs text-purple-600">24/7 helpline: 1-800-HELP</p>
            </div>
            <div className="space-y-1 md:space-y-2">
              <h4 className="font-medium text-purple-800 text-xs md:text-sm">Legal Resources</h4>
              <p className="text-xs text-purple-600">Free legal consultation available</p>
            </div>
          </div>
        </CardContent>
      </Card>
